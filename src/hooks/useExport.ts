...
export default () => {
  ...
  const exportSpecificFile = (_slides: Slide[]) => {
    const blob = new Blob([encrypt(JSON.stringify(_slides))], {
      type: "",
    });
    const file = new File([blob], "presentation.pptist", {
      type: "",
    });
    return file;
  };
  ...
  const exportPPTX = async (
      ...
      for (const el of slide.elements) {
        if (el.type === "text") {
          ...
          if (el.special) {
            ...
            let fillColor = formatColor(
              el.fill === "none" ? "#00000000" : el.fill
            );
            if (el.gradient) {
    ...
    const binaryData = await pptx.write({ outputType: "arraybuffer" });
