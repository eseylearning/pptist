<template>
  <div class="element-filter">
    <div class="row">
      <div style="flex: 2;">{{ t('ppt.enableFilter') }}</div>
    ...
    <template v-if="hasFilters">
      <div class="presets">
        <div class="preset-item" v-for="(item, index) in presetFilters" :key="index" @click="applyPresetFilters(item.values)">
          ...
          <span class="preset-label">{{ t(`ppt.${item.label}`) }}</span>
        </div>
      </div>
      <div class="filter">
        <div class="filter-item" v-for="filter in filterOptions" :key="filter.key">
          <div class="name">{{ t(`ppt.${filter.label}`) }}</div>
          <Slider
            class="filter-slider"
            :max="filter.max"
            :min="0"
...
const defaultFilters: FilterOption[] = [
  ...
  { label: 'hueRotate', key: 'hue-rotate', default: 0, value: 0, unit: 'deg', max: 360, step: 10 },
...
const presetFilters: {
  ...
  { label: 'blackWhite', values: { 'grayscale': '100%' } },
