<template>
  <div class="link-handler" :style="{ top: height * canvasScale + 10 + 'px' }">
    ...
    <a class="link" v-else @click="turnTarget(link.target)">{{ t('ppt.slidePage') }} {{link.target}}</a>
    <div class="btns">
      <div class="btn" @click="openLinkDialog()">{{ t('ppt.replace') }}</div>
      <Divider type="vertical" />
      <div class="btn" @click="removeLink(elementInfo)">{{ t('ppt.remove') }}</div>
    </div>
  </div>
</template>
